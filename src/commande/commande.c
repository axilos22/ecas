#include "commande.h"
	...
	unsigned int commande_ajile = 2048;

int calcul(int pos_ajile, int theta_ajile) {
	printk("pos_ajile = %d, theta_ajile = %d\n", pos_ajile, theta_ajile);
	y1_pos_V = ((pos_ajile*17.0)/4096.0);	
	y1_pos_m = ((y1_pos_V*0.92)/17.0)-.46;
	/*y1_pos_m = ((y1_pos_V*1.84/17.0)-.92);*/

	y2_theta_V = ((theta_ajile*7.2)/4096.0);
	y2_theta_rad = (((y2_theta_V*0.698)/7.2)-.349);

	/* ATTENTION: SECURITE A NE PAS SUPPRIMER */
	/*if (((y2_theta_V > 3.7)||(y2_theta_V < -3.5))) {
		commande_ajile = 2048;
		printk("secu\n");
	...
	/* Format fonctionnel */
	commande_ajile = (unsigned int) ((commande + 10) * 4096) / 20;	
	/*printk("cmd ajile= %d \n", commande_ajile);*/
	return commande_ajile;
}
